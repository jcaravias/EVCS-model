...
class Model_World(): 
    ...
    def get_charger_placement(self, scenario, n):
        """ returns a dictionary where the key is the node id and the value is the number of chargers at that node """ 
        
        charger_placement = {i:0 for i in range(self.total_nodes)}
        
        if scenario == 1: # scenario 1 is that there are chargers at all homes plus n additional chargers to be randomly placed at stores/work
            for i in range(self.no_homes): 
                charger_placement[i] += 1
                
            #randomly select n additional nodes from remaining work and store nodes 
            work_store_nodes = self.node_types["work"] + self.node_types["stores"] 
            random_placement = [random.choice(work_store_nodes) for i in range(n)]
            for i in random_placement:
                charger_placement[i] += 1
                
            return charger_placement
        
        if scenario == 2: #scenario 2 places all n chargers randomly (note this causes the issue that some vehicles may never visit a node with a charger)
            nodes = [i for i in range(self.total_nodes)] 
            random_placement = [random.choice(nodes) for i in range(n)] 
            for i in random_placement:
                charger_placement[i] += 1
            return charger_placement
